...
export default function PatientMainDashboard() {
    ...
    const fetchAppointments = async () => {
        const appointmentsResponse = await api.get(`/api/appointments/patient/${userEmail}`);
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                const [doctorsResponse, appointmentsResponse, prescriptionsResponse] = await Promise.all([
                    api.get('/api/doctors/available'),
                    api.get(`/api/appointments/patient/${userEmail}`),
                    api.get(`/api/prescriptions/patient/${userEmail}`)
                ]);

                setDoctors(normalizeListResponse(doctorsResponse.data));
                        ...
                        <div className="mt-4 space-y-3">
                            {filteredDoctors.length === 0 ? (
                                ...
                                filteredDoctors.map((doctor) => (
                                    <article key={doctor.id || doctor.doctorId || doctor.email} className="rounded-xl border border-slate-700 bg-slate-900/70 p-4">
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                            <div>
                                                <h3 className="font-bold text-slate-100">{doctor.name || doctor.fullName || 'Doctor'}</h3>
                                                <p className="text-sm text-cyan-300">{doctor.specialty || doctor.specialization || 'General'}</p>
                                                <p className="text-sm text-slate-300">{doctor.hospital || doctor.clinic || doctor.location || 'Clinic not specified'}</p>
                                            </div>
                                            <div className="text-left sm:text-right">
                                                <p className="text-sm text-slate-300">Available: {doctor.available || doctor.availability || 'Not specified'}</p>
                                                <p className="text-sm font-semibold text-emerald-300">LKR {(doctor.feeLkr || doctor.consultationFee || 0).toLocaleString()}</p>
                                            </div>
                                        </div>
                                        <div className="mt-3 flex flex-wrap gap-2">
                                            <button
                                                type="button"
                                                onClick={() => handleBookAppointment(doctor.id || doctor.doctorId, doctor.feeLkr || doctor.consultationFee)}
                        ...
                        <section className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur">
                            ...
                            <div className="mt-4 space-y-3 text-sm">
                                {appointments.map((appointment) => (
                                    <div key={appointment.id || appointment.appointmentId} className="rounded-xl border border-slate-700 bg-slate-900/70 p-3">
                                        <p className="font-semibold text-slate-100">{appointment.doctor || appointment.doctorName || 'Doctor'}</p>
                                        <p className="text-slate-300">{appointment.specialty || appointment.specialization || 'General'}</p>
                                        <p className="text-slate-300">{appointment.date || appointment.appointmentDate || '-'} at {appointment.time || appointment.appointmentTime || '-'}</p>
                                        <p className={`mt-1 inline-block rounded-md px-2 py-0.5 text-xs font-semibold ${(appointment.status || '').toLowerCase() === 'confirmed' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300'}`}>
                                            {appointment.status || 'Pending'} • {appointment.mode || appointment.consultationMode || 'Video'}
                                        </p>
                                    </div>
                                ))}
                            </div>
                    ...
                    <section className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur">
                        ...
                        <div className="mt-4 space-y-3 text-sm">
                            {prescriptions.map((prescription) => (
                                <article key={prescription.id || prescription.prescriptionId} className="rounded-xl border border-slate-700 bg-slate-900/70 p-3">
                                    <p className="font-semibold text-slate-100">{prescription.id || prescription.prescriptionId || 'RX'} • {prescription.doctor || prescription.doctorName || 'Doctor'}</p>
                                    <p className="text-slate-300">Issued: {prescription.issuedOn || prescription.issuedDate || '-'}</p>
                                    <p className="text-slate-200">{prescription.notes || prescription.instructions || 'No notes available.'}</p>
                                </article>
                            ))}
                        </div>
                    </section>
